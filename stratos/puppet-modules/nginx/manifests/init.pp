...
class nginx(
  ...
  $deployment_code = 'nginx-extension'
  $carbon_version  = $version
  $service_code    = 'nginx-extension'
  ...
  nginx::initialize { $deployment_code:
    repo      => $package_repo,
  ...
  nginx::push_templates {
    $service_templates:
      target    => $carbon_home,
      require   => Nginx::Initialize[$deployment_code];
  }
  ...
  nginx::start { $deployment_code:
    ...
    require => [
      Package['nginx'],
      Nginx::Push_templates[$service_templates],
    ];
  }

  # install stratos_base before java before haproxy before agent
  Class['stratos_base'] -> Class['java'] -> Class['python_agent'] -> Class['nginx']
}
