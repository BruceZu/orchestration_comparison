...
define nginx::initialize ($repo, $version, $service, $local_dir, $target, $owner,) {
  file {
    "/${local_dir}/apache-stratos-${service}-${version}.zip":
      ensure => present,
      source => "puppet:///modules/nginx/apache-stratos-${service}-${version}.zip";
  }
  ...
  exec {
    ...
    "downloading_stratos${service}-${version}.zip_for_${name}":
      ...
      command => "puppet:///modules/nginx/apache-stratos-${service}-${version}.zip", 
 ...
 file { "/${target}/apache-stratos-${service}-${version}/lib":
    ...
    source => ["puppet:///modules/nginx/${mb_type}"],
    sourceselect => all,
    recurse => true,
  }
}
